...
from src.path import Path, PawnMove, Astar, TRACE
from src.utils.player_exception import *


class Board(IDrawable):

    # TODO = Construcción del área
    ...
    def __init__(self, game, cols, rows, squareSize, innerSize):
        ...
        self.grid = [
            [Square(self, GridCoordinates(col, row)) for row in range(rows)] for
            col in range(cols)]
        self.width, self.height = squareSize * cols + innerSize * (
                cols - 1), squareSize * rows + innerSize * (rows - 1)
        if INTERFACE:
            self.window = GraphWin("Quoridor - UPC".format(self), self.width,
                                   self.height)
        ...
        for col in range(self.cols):
            for row in range(self.rows):
                ...
                if col != self.lastCol and row != self.firstRow:
                    self.storedValidFencePlacings.append(
                        FencePlacing(coord, Fence.DIRECTION.HORIZONTAL))
                if col != self.firstCol and row != self.lastRow:
                    self.storedValidFencePlacings.append(
                        FencePlacing(coord, Fence.DIRECTION.VERTICAL))
                coordValidPawnMoves, coordValidPawnMovesIgnoringPawns = [], []
                if col != self.firstCol:
                    coordValidPawnMovesIgnoringPawns.append(
                        PawnMove(coord, coord.left()))
                    if col == self.firstCol + 1 and row == self.middleRow:  # left pawn
                        coordValidPawnMoves.append(
                            PawnMove(coord, coord.left().top(), coord.left()))
                        coordValidPawnMoves.append(
                            PawnMove(coord, coord.left().bottom(),
                                     coord.left()))
                    elif col == self.middleCol + 1 and (
                            row == self.firstRow or row == self.lastRow):  # top and bottom pawns
                        coordValidPawnMoves.append(
                            PawnMove(coord, coord.left().left(), coord.left()))
                    else:
                        coordValidPawnMoves.append(
                            PawnMove(coord, coord.left()))
                if col != self.lastCol:
                    coordValidPawnMovesIgnoringPawns.append(
                        PawnMove(coord, coord.right()))
                    if col == self.lastCol - 1 and row == self.middleRow:  # right pawn
                        coordValidPawnMoves.append(
                            PawnMove(coord, coord.right().top(), coord.right()))
                        coordValidPawnMoves.append(
                            PawnMove(coord, coord.right().bottom(),
                                     coord.right()))
                    elif col == self.middleCol - 1 and (
                            row == self.firstRow or row == self.lastRow):  # top and bottom pawns
                        coordValidPawnMoves.append(
                            PawnMove(coord, coord.right().right(),
                                     coord.right()))
                    else:
                        coordValidPawnMoves.append(
                            PawnMove(coord, coord.right()))
                if row != self.firstRow:
                    coordValidPawnMovesIgnoringPawns.append(
                        PawnMove(coord, coord.top()))
                    if col == self.middleCol and row == self.firstRow + 1:  # top pawn
                        coordValidPawnMoves.append(
                            PawnMove(coord, coord.top().left(), coord.top()))
                        coordValidPawnMoves.append(
                            PawnMove(coord, coord.top().right(), coord.top()))
                    elif (
                            col == self.firstCol or col == self.lastCol) and row == self.middleRow + 1:  # left and right pawns
                        coordValidPawnMoves.append(
                            PawnMove(coord, coord.top().top(), coord.top()))
                    else:
                        coordValidPawnMoves.append(PawnMove(coord, coord.top()))
                if row != self.lastRow:
                    coordValidPawnMovesIgnoringPawns.append(
                        PawnMove(coord, coord.bottom()))
                    if col == self.middleCol and row == self.lastRow - 1:  # bottom pawn
                        coordValidPawnMoves.append(
                            PawnMove(coord, coord.bottom().left(),
                                     coord.bottom()))
                        coordValidPawnMoves.append(
                            PawnMove(coord, coord.bottom().right(),
                                     coord.bottom()))
                    elif (
                            col == self.firstCol or col == self.lastCol) and row == self.middleRow - 1:  # left and right pawns
                        coordValidPawnMoves.append(
                            PawnMove(coord, coord.bottom().bottom(),
                                     coord.bottom()))
                    else:
                        coordValidPawnMoves.append(
                            PawnMove(coord, coord.bottom()))
                self.storedValidPawnMoves[coord], \
                self.storedValidPawnMovesIgnoringPawns[
                    coord] = coordValidPawnMoves, coordValidPawnMovesIgnoringPawns

    def draw(self):
        if not INTERFACE:
            return
        background = Rectangle(Point(0, 0), Point(self.width, self.height))
        background.setFill(Color.RED.value)
    ...
    def hasFenceAtLeft(self, coord):
        for fence in self.fences:
            if fence.direction == Fence.DIRECTION.VERTICAL and (
                    fence.coord == coord or fence.coord == coord.top()):
                return True
        return False
    ...
    def hasFenceAtTop(self, coord):
        for fence in self.fences:
            if fence.direction == Fence.DIRECTION.HORIZONTAL and (
                    fence.coord == coord or fence.coord == coord.left()):
                return True
        return False
    ...
    def validPawnMoves(self, coord, ignorePawns=False):
        ...
        if not self.isAtLeftEdge(coord) and not self.hasFenceAtLeft(coord):
            ...
            if ignorePawns or not self.hasPawn(leftCoord):
                ...
                if not self.isAtLeftEdge(leftCoord) and not self.hasFenceAtLeft(
                        leftCoord) and not self.hasPawn(leftCoord.left()):
                    validMoves.append(
                        PawnMove(coord, leftCoord.left(), leftCoord))
                else:
                    if not self.isAtTopEdge(
                            leftCoord) and not self.hasFenceAtTop(
                        leftCoord) and not self.hasPawn(leftCoord.top()):
                        validMoves.append(
                            PawnMove(coord, leftCoord.top(), leftCoord))
                    if not self.isAtBottomEdge(
                            leftCoord) and not self.hasFenceAtBottom(
                        leftCoord) and not self.hasPawn(leftCoord.bottom()):
                        validMoves.append(
                            PawnMove(coord, leftCoord.bottom(), leftCoord))
        if not self.isAtRightEdge(coord) and not self.hasFenceAtRight(coord):
            ...
            if ignorePawns or not self.hasPawn(rightCoord):
                ...
                if not self.isAtRightEdge(
                        rightCoord) and not self.hasFenceAtRight(
                    rightCoord) and not self.hasPawn(rightCoord.right()):
                    validMoves.append(
                        PawnMove(coord, rightCoord.right(), rightCoord))
                else:
                    if not self.isAtTopEdge(
                            rightCoord) and not self.hasFenceAtTop(
                        rightCoord) and not self.hasPawn(rightCoord.top()):
                        validMoves.append(
                            PawnMove(coord, rightCoord.top(), rightCoord))
                    if not self.isAtBottomEdge(
                            rightCoord) and not self.hasFenceAtBottom(
                        rightCoord) and not self.hasPawn(
                        rightCoord.bottom()):
                        validMoves.append(
                            PawnMove(coord, rightCoord.bottom(), rightCoord))
        if not self.isAtTopEdge(coord) and not self.hasFenceAtTop(coord):
            ...
            if ignorePawns or not self.hasPawn(topCoord):
                ...
                if not self.isAtTopEdge(topCoord) and not self.hasFenceAtTop(
                        topCoord) and not self.hasPawn(topCoord.top()):
                    validMoves.append(PawnMove(coord, topCoord.top(), topCoord))
                else:
                    if not self.isAtLeftEdge(
                            topCoord) and not self.hasFenceAtLeft(
                        topCoord) and not self.hasPawn(topCoord.left()):
                        validMoves.append(
                            PawnMove(coord, topCoord.left(), topCoord))
                    if not self.isAtRightEdge(
                            topCoord) and not self.hasFenceAtRight(
                        topCoord) and not self.hasPawn(topCoord.right()):
                        validMoves.append(
                            PawnMove(coord, topCoord.right(), topCoord))
        if not self.isAtBottomEdge(coord) and not self.hasFenceAtBottom(coord):
            ...
            if ignorePawns or not self.hasPawn(bottomCoord):
                ...
                if not self.isAtBottomEdge(bottomCoord) and \
                    ...
                    if not self.isAtRightEdge(
                            bottomCoord) and not self.hasFenceAtRight(
                        bottomCoord) and not self.hasPawn(
                        bottomCoord.right()):
                        validMoves.append(
                            PawnMove(coord, bottomCoord.right(), bottomCoord))
        return validMoves
    ...
    def isValidPawnMove(self, fromCoord, toCoord, validMoves=None,
        ...
        if validMoves is None:
            validMoves = self.storedValidPawnMovesIgnoringPawns[
                ...
                fromCoord]  # self.validPawnMoves(fromCoord)
    ...
    def displayValidPawnMoves(self, player, validMoves=None):
        if not INTERFACE:
            return
        if validMoves is None:
            validMoves = self.storedValidPawnMoves[
                player.pawn.coord]  # self.validPawnMoves(player.pawn.coord)
        for validMove in validMoves:
    ...
    def validFencePlacings(self):
        ...
        for col in range(self.cols):
            for row in range(self.rows):
                if (self.isValidFencePlacing(GridCoordinates(col, row),
                                             Fence.DIRECTION.HORIZONTAL)):
                    validPlacings.append(FencePlacing(GridCoordinates(col, row),
                                                      Fence.DIRECTION.HORIZONTAL))
                if (self.isValidFencePlacing(GridCoordinates(col, row),
    ...
    def isValidFencePlacing(self, coord, direction):
        ...
        if not self.isAtTopEdge(coord) and not self.isAtRightEdge(
                coord) and direction == Fence.DIRECTION.HORIZONTAL and not self.hasFenceAtTop(
            coord) and not self.hasFenceAtTop(coord.right()):
        ...
        if not self.isAtLeftEdge(coord) and not self.isAtBottomEdge(
                coord) and direction == Fence.DIRECTION.VERTICAL and not self.hasFenceAtLeft(
            coord) and not self.hasFenceAtLeft(coord.bottom()):
    ...
    def displayValidFencePlacings(self, player, validPlacings=None):
        ...
        for validPlacing in validPlacings:
            ...
            possibleFence.draw(Color.Darker(player.color.value))
            del possibleFence
    ...
    def getFencePlacingFromMousePosition(self, x, y) -> FencePlacing:
        ...
        if x % fullSize > self.squareSize and y % fullSize < self.squareSize:
            ...
            return FencePlacing(square.coord,
                                Fence.DIRECTION.VERTICAL) if self.isValidFencePlacing(
                square.coord, Fence.DIRECTION.VERTICAL) else None
        # horizontal fence
        if x % fullSize < self.squareSize and y % fullSize > self.squareSize:
            square = self.getSquareFromMousePosition(x, y + self.squareSize)
            return FencePlacing(square.coord,
                                Fence.DIRECTION.HORIZONTAL) if self.isValidFencePlacing(
                square.coord, Fence.DIRECTION.HORIZONTAL) else None
        # on inner crossing space
        if x % fullSize > self.squareSize and y % fullSize > self.squareSize:
            square = self.getSquareFromMousePosition(x + self.squareSize,
                                                     y + self.squareSize)
            direction = Fence.DIRECTION.HORIZONTAL if square.left - x < square.top - y else Fence.DIRECTION.VERTICAL
            return FencePlacing(square.coord,
                                direction) if self.isValidFencePlacing(
                square.coord, direction) else None
        return None
        ...
        for player in self.game.players:
            ...
            if player.name == 'ElioBot' or player.name == 'RichiBot':
                ...
                path = Path.Dijkstra(self, player.pawn.coord,
                                     player.endPositions,
                                     ignorePawns=True)
                # path = Astar(self, player.pawn.coord,player.endPositions)
    ...
    def updateStoredValidPawnMovesIgnoringPawnsAt(self, coord):
        self.storedValidPawnMovesIgnoringPawns[coord] = self.validPawnMoves(
            coord, True)

    def removeIfExistStoredValidFencePlacing(self, fencePlacing):
        if fencePlacing in self.storedValidFencePlacings: self.storedValidFencePlacings.remove(
            fencePlacing)

    def updateStoredValidPawnMovesAfterPawnMove(self, fromCoord, toCoord):
        coords = [
            fromCoord] if fromCoord is not None else []  # fromCoord is None at start
    ...
    def updateStoredValidFencePlacingsAfterFencePlacing(self, coord, direction):
        ...
        self.removeIfExistStoredValidFencePlacing(
            FencePlacing(coord, direction))
        self.removeIfExistStoredValidFencePlacing(
            FencePlacing(coord.top() if direction == v else coord.left(),
                         direction))
        self.removeIfExistStoredValidFencePlacing(
            FencePlacing(coord.bottom() if direction == v else coord.right(),
                         direction))
        self.removeIfExistStoredValidFencePlacing(FencePlacing(
            coord.bottom().left() if direction == v else coord.top().right(),
            h if direction == v else v))
    ...
    def updateStoredValidPawnMovesIgnoringPawnsAfterFencePlacing(self, coord,
                                                                 direction):
        v, h = Fence.DIRECTION.VERTICAL, Fence.DIRECTION.HORIZONTAL
        ...
        for col in range(minCol, maxCol + 1):
            for row in range(minRow, maxRow + 1):
                self.updateStoredValidPawnMovesIgnoringPawnsAt(
    ...
    def drawOnConsole(self):
        ...
        for col in range(1, self.cols):
            ...
            pawn = self.getPawnAt(coord)
            print("%s%s" % (" " if not self.hasFenceAtLeft(coord) else "|",
                            " " if pawn is None else pawn.player.name[:1]),
                  end="")
        print("|")
        for row in range(1, self.rows):
            ...
            for col in range(self.cols):
                coord = GridCoordinates(col, row)
                print("%s+" % (" " if not self.hasFenceAtTop(coord) else "-"),
                      end="")
            print("")
            coord = GridCoordinates(0, row)
            pawn = self.getPawnAt(coord)
            print("|%s" % (" " if pawn is None else pawn.player.name[:1]),
                  end="")
            for col in range(1, self.cols):
                coord = GridCoordinates(col, row)
                pawn = self.getPawnAt(coord)
