...
class RandomBot(IBot):
    def moveRandomly(self, board) -> IAction:
        validPawnMoves = board.storedValidPawnMoves[
            self.pawn.coord]  # board.validPawnMoves(self.pawn.coord)
    ...
    def play(self, board) -> IAction:
        ...
        if random.randint(0, 2) == 0 and self.remainingFences() > 0 and len(
                board.storedValidFencePlacings) > 0:
